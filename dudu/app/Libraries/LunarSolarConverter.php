...
class LunarSolarConverter
{
    ...
    // lunar object convert to date format 'Y-m-d'
    public static function lunarToDate($lunar){
        $date = '';
        $date = $date . $lunar->lunarYear . '-' . $lunar->lunarMonth . '-' . $lunar->lunarDay;
        return date('Y-m-d', strtotime($date));
    }

    public static function lunarDateToSolarDate($date){
        $lunar = new Lunar();
        $lunar->lunarYear = date('Y', strtotime($date));
        $lunar->lunarMonth = date('m', strtotime($date));
        $lunar->lunarDay = date('d', strtotime($date));
        $solar = self::LunarToSolar($lunar);
        return $solar->solarYear.'-'.$solar->solarMonth.'-'.$solar->solarDay;
    }

    // "20190101" => "2019-01-01"
    public static function dateStrToDateFormat($dateStr){
        return substr($dateStr, 0, 4).'-'.substr($dateStr, 4, 2).'-'.substr($dateStr, 6, 2);
    }

    public static function dateStrToSolar($dateStr){
        $solar = new Solar();
        $solar->solarYear = substr($dateStr, 0, 4);
        $solar->solarMonth = substr($dateStr, 4, 2);
        $solar->solarDay = substr($dateStr, 6, 2);
        return $solar;
    }
}
